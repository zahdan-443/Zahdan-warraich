...
export const TripCostView: React.FC<TripCostViewProps> = ({
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      // Header
      doc.setFillColor(139, 157, 119); // #8b9d77
      doc.rect(0, 0, 210, 35, 'F');
      
      doc.setTextColor(255, 255, 255);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(22);
      doc.text("AL-HADI GOODS TRANSPORT", 14, 18);
      
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      doc.text("Official Freight & Trip Expense Waybill Estimate", 14, 26);
      
      doc.setTextColor(80, 80, 80);
      doc.setFontSize(10);
      doc.text(`Date Generated: ${new Date().toLocaleDateString('en-PK')} ${new Date().toLocaleTimeString('en-PK')}`, 14, 46);
      doc.text(`Route Distance: ${lastCalc.dist} KM (${lastCalc.isReturn ? 'Return Trip' : 'One-Way'})`, 14, 53);
      ...
      doc.text("Expense & Operational Breakdown", 14, 68);
      ...
      addRow("Fuel Rate / Ltr:", `${fuelPrice || 'N/A'} PKR / Ltr`);
      addRow("Vehicle Mileage:", `${mileage} KM / Ltr`);
      addRow("Estimated Fuel Consumption:", `${lastCalc.consumed} Liters`);
      addRow("Total Fuel Expense:", `PKR ${lastCalc.fuelCost.toLocaleString()}`);
      addRow("Motorway & Toll Tax:", `PKR ${lastCalc.toll.toLocaleString()}`);
      addRow("Driver Allowance / Kharcha:", `PKR ${lastCalc.driver.toLocaleString()}`);
      if (lastCalc.loading > 0) {
        addRow("Loading / Unloading Labor:", `PKR ${lastCalc.loading.toLocaleString()}`);
      }
      if (lastCalc.other > 0) {
        addRow("Misc (Challan / Repair / Maintenance):", `PKR ${lastCalc.other.toLocaleString()}`);
      }
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(74, 74, 53);
      doc.text("TOTAL ESTIMATED WAYBILL EXPENSE:", 20, y + 8);
      doc.setFontSize(16);
      doc.setTextColor(34, 139, 34);
      doc.text(`PKR ${lastCalc.total.toLocaleString()}`, 135, y + 8);
      
      y += 35;
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(100, 100, 100);
      doc.text("Generated by Al-Hadi Goods Transport Fleet Management System (Samundri, Punjab) | 03005370443", 14, y);
      doc.setFont('helvetica', 'italic');
      doc.text("this is a computer-generated document, no signature is required.", 14, y + 8);
      
      doc.save(`Waybill_Estimate_${lastCalc.dist}km_${Date.now()}.pdf`);
    } catch (err) {
      console.error(err);
      ...
      {onNavigate && (
        <div className="mb-6">
          <button
            ...
            className="inline-flex items-center gap-3 px-6 py-3.5 bg-[#4a4a35] hover:bg-[#383827] text-white rounded-2xl font-bold text-base shadow-md transition-all active:scale-95 cursor-pointer"
          >
            <span className="text-xl">⬅</span>
            <span>{lang === 'ur' ? 'مرکزی صفحہ پر واپس جائیں' : 'Back to Dashboard'}</span>
          </button>
        </div>
      )}
      
      {/* Header Container */}
      <div className="bg-white p-6 sm:p-8 md:p-10 rounded-[36px] shadow-sm border border-[#ecece0] mb-8">
        <header className="mb-8 pb-6 border-b border-[#ecece0]">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-[#4a4a35]">
            {lang === 'ur' ? 'سفر اور کرایہ کا آسان کیلکولیٹر' : 'Simple Trip Expense Calculator'}
          </h1>
          <p className="text-sm text-[#8e8e75] font-sans mt-1">
            {lang === 'ur' ? 'ڈرائیوروں اور عملے کے لئے آسان خرچہ حساب' : 'Easy logistics cost calculator for drivers and staff'}
          </p>
        </header>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="p-6 rounded-3xl bg-[#fdfbf7] border border-[#ecece0] space-y-5">
                <h3 className="font-serif font-bold text-lg text-[#4a4a35] flex items-center gap-2 border-b border-[#ecece0] pb-3">
                  ...
                  {lang === 'ur' ? 'فیول اور فاصلہ پیرامیٹرز (دستی اندراج)' : 'Fuel & Route Parameters (Manual Input)'}
                </h3>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-2">
                      {lang === 'ur' ? 'فیول ریٹ (روپے فی لیٹر)' : 'Fuel Rate (PKR / Ltr)'}
                    </label>
                    <input
                      type="number"
                      value={fuelPrice}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-2">
                      {lang === 'ur' ? 'گاڑی کی اوسط (مائلیج Km/L)' : 'Vehicle Mileage (Km / Ltr)'}
                    </label>
                    <input
                      type="number"
                      value={mileage}
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-2">
                    {lang === 'ur' ? 'کل فاصلہ (کلومیٹر)' : 'Route Distance (Kilometers)'}
                  </label>
                  <input
                    type="number"
                    value={distance}
              ...
              <div className="p-6 rounded-3xl bg-[#fdfbf7] border border-[#ecece0] space-y-4">
                <h3 className="font-serif font-bold text-lg text-[#4a4a35] flex items-center gap-2 border-b border-[#ecece0] pb-3">
                  ...
                  {lang === 'ur' ? 'دیگر سفری اخراجات' : 'Operational & Route Expenses'}
                </h3>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-1.5">
                      {lang === 'ur' ? 'ٹول ٹیکس اور موٹروے' : 'Toll Tax & Motorway (PKR)'}
                    </label>
                    <input
                      type="number"
                      value={toll}
                      ...
                      placeholder="1200"
                      className="w-full bg-white border border-[#ecece0] rounded-xl px-3.5 py-2.5 text-sm font-semibold text-[#4a4a35] focus:border-[#8b9d77] focus:outline-none font-mono"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-1.5">
                      {lang === 'ur' ? 'ڈرائیور الاؤنس / خرچہ' : 'Driver Allowance (PKR)'}
                    </label>
                    <input
                      type="number"
                      value={driverAllow}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-1.5">
                      {lang === 'ur' ? 'لوڈنگ / ان لوڈنگ مزدوری' : 'Loading / Labor Charges (PKR)'}
                    </label>
                    <input
                      type="number"
                      value={loading}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-[#8e8e75] mb-1.5">
                      {lang === 'ur' ? 'متفرق اخراجات (چالان، مرمت)' : 'Misc (Challan / Repair / Maintenance)'}
                    </label>
                    <input
                      type="number"
                      value={other}
                  ...
                  <div className="pt-8 space-y-3">
                    {saveSuccess ? (
                      ...
                      <button
                        ...
                        className="w-full py-3.5 bg-[#f0f0e4] hover:bg-[#8b9d77] hover:text-white text-[#5a5a40] border border-[#d8d8c0] rounded-full text-xs sm:text-sm font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2"
                      >
                        <BookmarkPlus className="w-4 h-4" />
                        <span>{lang === 'ur' ? 'ڈائری میں محفوظ کریں (Record to Diary)' : 'Record to Diary'}</span>
                      </button>
                    )}
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-white border border-[#ecece0] hover:border-[#25D366] hover:text-[#25D366] text-[#8e8e75] rounded-full text-xs sm:text-sm font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2"
                    >
                      <Share2 className="w-4 h-4 text-[#25D366]" />
                      <span>{lang === 'ur' ? 'واٹس ایپ پر شیئر کریں (Share via WhatsApp)' : 'Share via WhatsApp'}</span>
                    </button>

                    <button
                      type="button"
                      onClick={handleExportPDF}
                      className="w-full py-3.5 bg-[#8b9d77]/15 hover:bg-[#8b9d77] hover:text-white text-[#5a5a40] border border-[#8b9d77]/40 rounded-full text-xs sm:text-sm font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2 shadow-2xs group"
                    >
                      <FileDown className="w-4 h-4 text-[#8b9d77] group-hover:text-white" />
                      <span>{lang === 'ur' ? 'پی ڈی ایف رسید (Export Official PDF Waybill)' : 'Export Official PDF Waybill'}</span>
                    </button>
                  </div>
                </div>
              ) : (
